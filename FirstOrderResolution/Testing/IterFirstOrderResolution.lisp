...
(defun Icompare (sentence clause)                 ;see if the complement of literal is in clause, return literal if it is (if literal is a negation (not "literal") it will not find the complement)
  ...
  (let ((factored (IfactorRecurse (car restclause) clause)))
    (if (eq factored nil)
        (if (equal (list-length restclause) 1)   
            clause
            (Ifactor clause (cdr restclause))
            )
        (InewClause clause clause factored)
        )
    )
  )
...
(defun Ifactorall (clauses)
  (let ((factored (Ifactor (car clauses) (car clauses))))
    (if (or (equal factored nil) (equal factored T))
        ...
        (if (equal (list-length clauses) 1) 
            (cons factored nil)
            (cons factored (Ifactorall (cdr clauses)))
            )
        )
    )
  )

(defun InewClause (c1 c2 complement)                         ;given two clauses and a shared complement pair, return the resolution of those clauses, or T if it evaluates to true
  (setf c1 (Icombine c2 c1))                                 ;concatenate the two clauses and store it in c1 without repeating literals
...
(defun IsecondClauseLoop (a copy)                                     ;second layer of the iteration
  (let ((new (Iresolve a (first copy)))                               ;resolve two clauses of the KB
    ...
    (if (equal new T)                                                ;if the resolved clause is true
        ...
        (if (equal (list-length copy) 1)                           ;if this is the last clause to check with a (the last recursive step)
              ...
              (if (equal new nil)                                    ;if there are no new clauses to add
                  (IsecondClauseLoop a (rest copy))                   ;return the next iteration of the recursion
                  (cons new (IsecondClauseLoop a (rest copy)))        ;return the new clauses concatenated onto the next iteration of the recursion
                  )
              )
        )
    )
...
(defun Icombine (list1 list2)                          ;combine two lists without repeating any elements
...
(defun IfirstClauseLoop (real copy)                                  ;first layer of recursive iteration
  (let ((a (first copy))                                            ;pick out a clause of the KB
    ...
    (setf new (IsecondClauseLoop a real))                            ;send a to secondClauseLoop to compare it with all the other clauses
    (if (equal (find T new) T)                                      ;if one of the new clauses is T
        T                                                           ;the KB resolved successfully so return T
        (progn
          (setf new (Icombine new real))                             ;Otherwise combine the new clauses with the KB
...
(defun IrecurseResolution (using saving maxlength currentlength)                                               ;takes in full KB including ~a
  (let ((resolved (IfirstClauseLoop using using)))                            ;save resolved KB into resolved
...
(defun Iextend (using saving maxlength currentlength)
  (let ((sorted (Isorter saving maxlength (+ currentlength 1))))
    (setf using (Icombine using (car sorted)))
    (setf saving (cadr sorted))
    (IrecurseResolution using saving (caddr sorted) (+ currentlength 1))
    )
  )
    

(defun Isorter (clauses max length)
  (let ((use nil)
        (save nil))
    (dolist (i clauses)
...
(defun Iresolution (KB s q)                           ;KB is the knowledge base, a is the current state and query (if "state", then "query")
  (setf KB (append s KB))                            ;add the state to the KB
   ...
   (IrecurseResolution KB nil 1 1)                            ;recursively resolve until the solution is found
)
